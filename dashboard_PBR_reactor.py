...
with tab1:
    ...
    Este simulador tem como objetivo ajudar a tornar visível e tangível a teoria abstrata dos livros didáticos (como o Fogler)
    para os alunos de engenharia química ao permitir alterar parâmetros (como temperatura, concentração inicial de reagente) e entender melhor como e procurar entender o por que a 
    alterações destes valores influencia no perfil de conversão ao longo do reator teórico.
    </p>
    ...
    st.image("https://raw.githubusercontent.com/gaappucrio/PBR_reactor_dashboard/refs/heads/main/Esterification_Reaction.png", caption="Reação de esterificação de ácido benzóico em benzoato de etila")
    ...
    situaram-se abaixo do limite crítico (<i>t<sub>ref</sub></i> = 1,657), indicando que o erro analítico do sistema inviabilizava a estimação única
    ...
    de reação ($k$) foi expressa em sua forma reparametrizada em torno de uma temperatura média de referência (<i>T<sub>M</sub></i> = 378,15 K):
    </p>
    """, unsafe_allow_html=True)
    ...
    os intervalos de confiança de <i>KP<sub>1</sub></i>, <i>KP<sub>2</sub></i> e da constante de adsorção da água (<i>K<sub>W</sub></i>) foram drasticamente reduzidos.
    </p>
    """, unsafe_allow_html=True)
    ...
    st.write(r"""
    <p style='font-size:20px'>
    Fator Pré-exponencial (<i>k<sub>0</sub></i>): 185,3 L^2/(<i>g<sub>cat</sub></i>.s.mol), 
    ...
    st.write(r"""
    <p style='font-size:20px'>
    Energia de Ativação Aparente (<i>E<sub>a</sub></i>): $68800 J/mol (ou $68,8 kJ/mol), valor altamente 
    consistente com dados independentes previamente validados na literatura (69,1 +- 0,5 kJ/mol).
    </p>
    """, unsafe_allow_html=True)
